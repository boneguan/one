...
class Upload extends BaseController
{
    ...
    /**
     * 文件上传，APP专用
     * 创建时间：2018年6月21日09:55:04
     *
     * @return Ambigous <\think\response\Json, string>
     */
    public function uploadFileForApp()
    {
        $title = '图片上传';
        if (empty($this->uid)) {
        ...
        if (empty($_FILES)) {
            return $this->outMessage($title, null, '-9999', "缺少文件");
        }
        ...
        foreach ($_FILES as $key => $value) {
            $this->file_name = $value["name"]; // 文件原名
            $this->file_size = $value["size"]; // 文件大小
            $this->file_type = $value["type"]; // 文件类型
            ...
            $file_name_explode = explode(".", $this->file_name); // 图片名称
            $suffix = count($file_name_explode) - 1;
            $guid = $file_name_explode[0] . "_" . time();
            $ext = "." . $file_name_explode[$suffix]; // 获取后缀名
            $newfile = $guid . $ext; // 重新命名文件
            
            $ok = $this->moveUploadFile($value["tmp_name"], $this->reset_file_path . $newfile);
            if ($ok["code"]) {
    ...
    public function test()
    {
        ...
        return $this->outMessage("测试上传", $arr);
    }
