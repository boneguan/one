...
class BaseController extends Controller
{
    ...
    public function init()
    {
        ...
        if ($check_auth) {
            ...
            if (! request()->isAjax()) {
                ...
                $root_module_info = $this->website->getSystemModuleInfoS($this->rootid, 'module_name,url,module_picture');
    ...
    public function getNavigation()
    {
        ...
        //查询用户的权限
        $user_quanxian = $this->user->getUserQuanxian($this->uid);
        
        $list = array();
        if($user_quanxian){   
        	$quanxian_arr = explode(",", $user_quanxian['module_id_array']);      	 
        	foreach ($first_list as $k => $v) {
        		$submenu = $this->user->getchildModuleQuery($v['module_id']);
        		if(in_array($v['module_id'], $quanxian_arr)){
        			$list[$k]['data'] = $v;
        		}
        		foreach ($submenu as $ke => $va){
        			if(in_array($va['module_id'], $quanxian_arr)){
        				$list[$k]['sub_menu'][$va['module_id']] = $va;
        			}
        		} 		
        	}
        }else{
