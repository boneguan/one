...
class Cms extends BaseController
{
    ...
    public function articleClassList()
    {
        ...
        $template_url["pc_template_url"] = "template/shop/" . $pc_template['value'] . '/Cms/';
        $template_url["wap_template_url"] = "template/wap/" . $wap_template['value'] . '/Cms/';
    ...
    public function addUpdateAritcleClass()
    {
        if (request()->isAjax()) {
            ...
            $extra['wap_custom_template'] = request()->post('wap_custom_template', '');
            $extra['pc_custom_template'] = request()->post('pc_custom_template', '');
            $extra['wap_custom_detail_template'] = request()->post('wap_custom_detail_template', '');
            $extra['pc_custom_detail_template'] = request()->post('pc_custom_detail_template', '');

            if ($class_id > 0) {
                $retval = $article->updateArticleClass($class_id, $name, $sort, $pid,$extra);
            } else {
                $retval = $article->addAritcleClass($name, $sort, $pid,$extra);
            }
