...
/**
 ...
 /**
     ...
     */
    public function bindWchat($unionid, $wx_info, $user_name)
    {
    	
    	$user_names = $this->user->getInfo(['user_name'=>$user_name]);
    	$user_email = $this->user->getInfo(['user_email'=>$user_name]);
    	$user_tel = $this->user->getInfo(['user_tel'=>$user_name]);
    	
    	if($user_names && $user_email && $user_tel){
    		$uid = $user_names['uid'];
    	}else if($user_names=="" && $user_email || $user_tel){
    		if($user_email){
    			$uid = $user_email['uid'];
    		}else{
    			$uid = $user_tel['uid'];
