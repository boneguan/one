...
use data\extend\alipay_new\AlipayNotify;
use data\extend\alipay_new\request\AlipayTradeOrderinfoSyncRequest;
use think\Log;
/**
...
//         \think\Loader::addNamespace('alipaynew', '../data/extend/alipaynew');
        // 获取支付宝支付参数(统一支付到平台账户)
        $alipay_config_service = new Config();
        $alipay_new_config = $alipay_config_service->getAlipayConfigNew(0);

        // 获取支付宝支付参数(统一支付到平台账户)
        $aop = new AopClient();
        $aop->gatewayUrl = 'https://openapi.alipay.com/gateway.do';
        $aop->appId = $alipay_new_config['value']['appid'];
    ...
    public function getAlipayConfig()
    {
        ...
        // 编码
        $alipay_config['postCharset'] = 'UTF-8';
        // 仅支持JSON
     ...
     */
    public function aliPayRefund($refund_no, $trade_no, $refund_fee, $out_trade_no)
    {
