...
class WebSite extends BaseService implements IWebSite
{
     ...
     * @param unknown $module_id            
     */
    public function getSystemModuleInfoS($module_id, $field = '*')
    {
        $res = $this->module->getInfo(array(
         'module_id' => $module_id
        ), "*");
        $admin_user = new AdminUserModel();
        $user = $admin_user->getInfo(["uid"=>$this->uid],"group_id_array");
        $auth_group = new AuthGroupModel();        
        $group_info = $auth_group->getInfo(['group_id'=>$user['group_id_array']]);

        $condition['module_id'] = [ '<>',$res['module_id']];
        $condition["url"] = $res['url'];
        $condition["is_control_auth"] = 1;
        $array = $this->module->pageQuery(1, 0, $condition, "level desc", "*");
        if(empty($array['data'])){
        	return $res;
        }else{
        	if(strpos($group_info['module_id_array'], $array['data'][0]['module_id']."") !==false){
        		return $res;
        	}else{
        		$array = $this->module->pageQuery(1, 0, ['pid'=>$res['module_id'] ], "sort asc", "*");
        		foreach ($array['data'] as $key => $val){
        	
        			if(strpos($group_info['module_id_array'], $val['module_id']."") !==false){
        				$res['url'] = $val['url'];
        				break;
        			}
        		}
