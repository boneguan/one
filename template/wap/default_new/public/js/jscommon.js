...
function isvalint1(intval) {
	var r = /^(-)?[0-9]*[0-9][0-9]*$/;
	if (r.test(intval) == false) return false;
	return true;
}
...
function datecompareVal(val1, val2) {
	...
	if (arr1.length != 3 || arr2.length != 3) {
		...
		//	判定obj1和obj2的大小
		if (oldtime1 > oldtime2) return 1;
		else if (oldtime1 < oldtime2) return -1;
		else return 0;
	}
}
...
function datecompare(obj1, obj2) {
	...
	if (r1 != null && r1.length == 4) {
		...
		if (d1.getFullYear() == r1[1] && d1.getMonth() == r1[2] && d1.getDate() == r1[3]) b1 = true;
	}
	if (b1 == false) {
		alert("您输入的不是有效的日期类型");
		obj1.focus();
	...
	if (r2 != null && r2.length == 4) {
		...
		if (d2.getFullYear() == r2[1] && d2.getMonth() == r2[2] && d2.getDate() == r2[3]) b2 = true;
	}
	if (b2 == false) {
		alert("您输入的不是有效的日期类型");
		obj2.focus();
